...
def restart_as_admin():
    ...
    try:
        ...
        # If elevation cannot be requested, continue non-elevated (useful for debugging)
        print("Elevation failed:", e)

# Dark stylesheet (compact spacing)
DARK_STYLESHEET = """
...
QTextEdit { background-color: #0f1112; border: 1px solid #252728; padding: 8px; font-family: "Consolas", monospace; font-size: 10pt; color: #dfeffb; border-radius: 6px; }
...
if __name__ == "__main__":
    # If not admin, request elevation (this will relaunch using pythonw.exe)
    if not is_admin():
        restart_as_admin()
